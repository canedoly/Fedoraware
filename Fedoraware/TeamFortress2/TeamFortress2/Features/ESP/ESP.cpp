	...
	for (const auto& Player : g_EntityCache.GetGroup(EGroupType::PLAYERS_ALL))
	{
		...
		if (GetDrawBounds(Player, vTrans, x, y, w, h))
		{
			...
			switch (Vars::ESP::Players::Box.Value)
			{
				case 1:
				{
					int height = h + 3; //don't ask me /shrug
					int width = w + 2;

					g_Draw.OutlinedRect(x, y, width, height, drawColor);
					if (Vars::ESP::Main::Outlinedbar.Value)
					{
						g_Draw.OutlinedRect(x, y, width, height, Colors::OutlineESP);
						g_Draw.OutlinedRect(x + 3, y + 4, width, height, Colors::OutlineESP);
					}
