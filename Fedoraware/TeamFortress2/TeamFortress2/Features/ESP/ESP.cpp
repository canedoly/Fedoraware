	...
	for (const auto& Player : g_EntityCache.GetGroup(EGroupType::PLAYERS_ALL))
	{
		...
		if (GetDrawBounds(Player, vTrans, x, y, w, h))
		{
			...
			if (I::Engine->GetPlayerInfo(nIndex, &pi))
			{
				...
				if (F::BadActors.MarkedCheaters[pi.friendsID] && Vars::ESP::Players::CheaterDetection.Value)
				{
			...
			if (Vars::ESP::Players::HealthBar.Value)
			{
				...
				{
					g_Draw.String(FONT_NAME, x - 2, (y + h) - (ratio * h) - 15, nHealth > nMaxHealth ? Colors::Overheal : healthColor, ALIGN_CENTERHORIZONTAL, "%d", nHealth);
				}
